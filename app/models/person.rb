...
class Person < ActiveRecord::Base
  ...
  def self.attendees_with_report_fields(start_day, end_day)
    date_range = start_day == end_day ? start_day : (start_day..(end_day+1))
    ...
    workshop_sessions = {}; WorkshopSession.all(:conditions => {:starts_at => date_range}).each { |r| workshop_sessions[r.id] = r }
